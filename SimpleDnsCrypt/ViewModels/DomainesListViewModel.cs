...
namespace SimpleDnsCrypt.ViewModels
{
	...
	public class DomainsListViewModel : Screen
	{
	    ...
	    public DomainsListViewModel(IWindowManager windowManager, IEventAggregator events)
	    {
		    ...
		    {
				//set default
				_domainBlacklistFile = Path.Combine(Directory.GetCurrentDirectory(), Global.DnsCryptProxyFolder, Global.DnsCryptProxyFileBlockedNames);
		    ...
		    {
				//set default
				_domainWhitelistRuleFilePath = Path.Combine(Directory.GetCurrentDirectory(), Global.DnsCryptProxyFolder, Global.DnsCryptProxyFileAllowedNames);
		    ...
		    {
				//set default
			    _domainBlacklistRuleFilePath = Path.Combine(Directory.GetCurrentDirectory(), Global.DnsCryptProxyFolder, Global.DnsCryptProxyFileBlockedNames);
			...
			try
			{
				if (_isBlacklistEnabled)
				{
					...
					if (dnscryptProxyConfiguration.Blocked_domains == null)
					{
						...
						{
							filter_file = _domainBlacklistFile,
					...
					if (string.IsNullOrEmpty(dnscryptProxyConfiguration.Blocked_domains.filter_file))
					{
						dnscryptProxyConfiguration.Blocked_domains.filter_file = _domainBlacklistFile;
						saveAndRestartService = true;
					}
				...
				else
				{
					...
					dnscryptProxyConfiguration.Blocked_domains.filter_file = null;
					DnscryptProxyConfigurationManager.DnscryptProxyConfiguration = dnscryptProxyConfiguration;
		...
		public void ChangeBlacklistFilePath()
		{
			try
			{
				...
				{
					DomainBlacklistFile = Path.Combine(blacklistFolderDialog.SelectedPath, Global.DnsCryptProxyFileBlockedNames);
				}
			}
			catch (Exception exception)
			{
		...
		public void ChangeWhitelistRulesFilePath()
		{
			try
			{
				...
				if (result == DialogResult.OK)
				{
					DomainWhitelistRuleFilePath = Path.Combine(whitelistFolderDialog.SelectedPath, Global.DnsCryptProxyFileAllowedNames);
				}
			}
			catch (Exception exception)
			{
		...
		public void ChangeBlacklistRulesFilePath()
		{
			try
			{
				...
				if (result == DialogResult.OK)
				{
					DomainBlacklistRuleFilePath = Path.Combine(blacklistFolderDialog.SelectedPath, Global.DnsCryptProxyFileBlockedNames);
				}
			}
			catch (Exception exception)
			{
