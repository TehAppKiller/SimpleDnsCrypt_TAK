...
{
	[Export(typeof(AllowedDomainsLogViewModel))]
	public class AllowedDomainsLogViewModel : Screen
	{
		...
		public AllowedDomainsLogViewModel()
		{
			_isLogging = false;
		...
		public bool IsLogging
		{
			...
			{
				_isLogging = value;
				AllowedDomainsLog(DnscryptProxyConfigurationManager.DnscryptProxyConfiguration);
				NotifyOfPropertyChange(() => IsLogging);
			}
		}

		private async void AllowedDomainsLog(DnscryptProxyConfiguration dnscryptProxyConfiguration)
		{
			...
			try
			{
				if (_isLogging)
				{
					...
					if (dnscryptProxyConfiguration.allowed_domains == null)
					{
						dnscryptProxyConfiguration.allowed_domains = new FilterList
						{
							log_file = Global.DnsCryptProxyLogAllowedNames,
							log_format = defaultLogFormat
						};
						saveAndRestartService = true;
					}

					if (string.IsNullOrEmpty(dnscryptProxyConfiguration.allowed_domains.log_format) ||
						!dnscryptProxyConfiguration.allowed_domains.log_format.Equals(defaultLogFormat))
					{
						dnscryptProxyConfiguration.allowed_domains.log_format = defaultLogFormat;
						saveAndRestartService = true;
					}

					if (string.IsNullOrEmpty(dnscryptProxyConfiguration.allowed_domains.log_file))
					{
						dnscryptProxyConfiguration.allowed_domains.log_file = Global.DnsCryptProxyLogAllowedNames;
						saveAndRestartService = true;
					}
					...
					LogFile = Path.Combine(Directory.GetCurrentDirectory(), Global.DnsCryptProxyFolder,
						dnscryptProxyConfiguration.allowed_domains.log_file);
						...
						await Task.Run((System.Action)(() =>
						{
								...
								FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))
							{
								...
								while (_isLogging)
								{
									...
									{
										var allowLogLine = new Models.DomainLogLine(line);
										AddLogLine(allowLogLine);
									}
				...
				{
					//disable allow log again
					_isLogging = false;
					dnscryptProxyConfiguration.allowed_domains.log_file = null;
					DnscryptProxyConfigurationManager.DnscryptProxyConfiguration = dnscryptProxyConfiguration;
