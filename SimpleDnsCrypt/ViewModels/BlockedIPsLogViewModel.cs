...
{
	[Export(typeof(BlockedIPsLogViewModel))]
	public class BlockedIPsLogViewModel : Screen
	{
		...
		public bool IsLogging
		{
			...
			{
				_isLogging = value;
				BlockedIPsLog(DnscryptProxyConfigurationManager.DnscryptProxyConfiguration);
				NotifyOfPropertyChange(() => IsLogging);
			}
		}

		private async void BlockedIPsLog(DnscryptProxyConfiguration dnscryptProxyConfiguration)
		{
			...
			try
			{
				if (_isLogging)
				{
					...
					if (dnscryptProxyConfiguration.blocked_ips == null)
					{
						dnscryptProxyConfiguration.blocked_ips = new FilterList
						{
							log_file = Global.DnsCryptProxyLogBlockedIPs,
							log_format = defaultLogFormat
						};
						saveAndRestartService = true;
					}

					if (string.IsNullOrEmpty(dnscryptProxyConfiguration.blocked_ips.log_format) ||
						!dnscryptProxyConfiguration.blocked_ips.log_format.Equals(defaultLogFormat))
					{
						dnscryptProxyConfiguration.blocked_ips.log_format = defaultLogFormat;
						saveAndRestartService = true;
					}

					if (string.IsNullOrEmpty(dnscryptProxyConfiguration.blocked_ips.log_file))
					{
						dnscryptProxyConfiguration.blocked_ips.log_file = Global.DnsCryptProxyLogBlockedIPs;
						saveAndRestartService = true;
					}
					...
					LogFile = Path.Combine(Directory.GetCurrentDirectory(), Global.DnsCryptProxyFolder,
						dnscryptProxyConfiguration.blocked_ips.log_file);
						...
						await Task.Run((System.Action)(() =>
						{
								...
								FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))
							{
								...
								while (_isLogging)
								{
									...
									{
										var blockLogLine = new Models.IPLogLine(line);
				...
				else
				{
					//disable log again
					_isLogging = false;
					dnscryptProxyConfiguration.blocked_ips.log_file = null;
					DnscryptProxyConfigurationManager.DnscryptProxyConfiguration = dnscryptProxyConfiguration;
